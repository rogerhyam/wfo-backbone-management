...
$sql = "SELECT 
	taxonID as wfo,
    `references`,
    concat('https://powo.science.kew.org/taxon/urn:lsid:ipni.org:names:', substring(`references`, 36 )) as uri,
    concat('urn:lsid:ipni.org:names:', substring(`references`, 36 )) as lsid
FROM botalista_dump_2 
WHERE `references` LIKE 'https://wcvp.science.kew.org/taxon/%';";
...
while($row = $result->fetch_assoc()){
    ...
    $display_text = "POWO record for {$row['lsid']}";
