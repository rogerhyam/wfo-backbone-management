...
/*
   ...
   php -d memory_limit=10G import_ipni_references.php
...
$sql = "SELECT 
	    b.taxonID as wfo, 
        concat('https://www.ipni.org/n/', trim(substr(b.scientificNameID, 25))) as linkUri,
        concat('IPNI record: ', trim(substr(b.scientificNameID, 25))) as displayText
    FROM 
        botalista_dump_2 as b
        JOIN identifiers as i on i.`value` = b.taxonID
        JOIN `names` as n on n.prescribed_id = i.id
    where scientificNameID like '%urn:lsid:ipni.org:names:%'
    limit 1500000 offset $offset;
    ";
